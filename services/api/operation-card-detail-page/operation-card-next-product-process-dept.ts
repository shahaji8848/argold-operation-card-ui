...
const GETOperationCardDetailNextProductProcessDepartment = async (product_process: any, token: any) => {
  // const url = `${CONSTANTS.API_BASE_URL}${CONSTANTS.STANDARD_API_PATH}/Karigar`;
  const fields: any = ['name', 'product_process', 'title', 'sequence'];

  const filters: any = [['product_process', '=', `${encodeURIComponent(product_process)}`]];
  const url = `${CONSTANTS.API_BASE_URL}${CONSTANTS.STANDARD_API_PATH}/Product Process Department?fields=${JSON.stringify(
    fields

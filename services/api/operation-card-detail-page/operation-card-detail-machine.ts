...
const GETOperationCardDetailMachine = async (
  ...
  const filters: any = [
    ['product_process_department', '=', `${product_process_department}`],
  ];
  const url = `${CONSTANTS.API_BASE_URL}${
    CONSTANTS.STANDARD_API_PATH
  }/Machine?fields=${JSON.stringify(fields)}&filters=${JSON.stringify(
    filters
  )}&limit=None`;
